        ...
        if self.w_rebild <= W and self.h_rebild <= H:
            if len(read_det) > 1:
                for i in range(len(read_det)):
                    ...
                    with open(f"{self.path_file_rebild}/{names}_{str(self.w_rebild)}x{str(self.h_rebild)}_{str(i)}.txt", "a") as file_2:
                        file_2.write(str(int(read_det[i][0])) + " " + str(detect_norm[0]) + " " + str(detect_norm[1]) + " " + str(detect_norm[2]) + " " + str(detect_norm[3]) + "\n")
                    ...
                    for j in range(len(read_det)):
                        ...
                        if (converture[0] + self.__pix_x_indent < int(x_cnt) < converture[2] - self.__pix_x_indent) and (converture[1] + self.__pix_y_indent < int(y_cnt) < converture[3] - self.__pix_y_indent) and (read_det[i] != read_det[j]):
                            ...
                            with open(f"{self.path_file_rebild}/{names}_{str(self.w_rebild)}x{str(self.h_rebild)}_{str(i)}.txt", "a") as file_2:
                                file_2.write(str(int(read_det[j][0])) + " " + str(det_norm[0]) + " " + str(det_norm[1]) + " " + str(det_norm[2]) + " " + str(det_norm[3]) + "\n")

                    cv.imshow("frame", img_copy[converture[1]:converture[3], converture[0]:converture[2]])
                    cv.waitKey(self.__time_delay)
            else:
                for i in range(len(read_det)):
                    rebilt_lst, converture = self.rebild_cordinate(read_det[i], size_image_W=W, size_image_H=H, w_n=self.w_rebild, h_n=self.h_rebild)
                    cv.imwrite(f"{self.path_file_rebild}/{names}_{str(self.w_rebild)}x{str(self.h_rebild)}_{str(i)}.jpg", img[converture[1]:converture[3], converture[0]:converture[2]])
            ...
            for i in range(len(read_det)):
                ...
                with open(f"{self.path_file_rebild}/{names}_{str(self.w_rebild)}x{str(self.h_rebild)}_{str(count_image)}.txt", "a") as file_2:
                    file_2.write(str(int(read_det[i][0])) + " " + str(detect_norm[0]) + " " + str(detect_norm[1]) + " " + str(detect_norm[2]) + " " + str(detect_norm[3]) + "\n")
            cv.imshow("frame", img)
            cv.waitKey(self.__time_delay)

    def walk_files(self):
        lst_file = os.listdir(self.path_file)
        count_image = 0
...
cls = Rebild_Data(r"temps_2", r"temp_reb", r"dataset_21_03_2023/no_sort", 900, 900)
cls.set_pix_x_indent(3)
cls.set_pix_y_indent(3)
cls.set_time_delay(1)
cls.set_x_rand_left(-10)
cls.set_x_rand_right(10)
cls.set_y_rand_left(-10)
cls.set_y_rand_right(10)
cls.walk_files()